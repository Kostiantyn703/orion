...
void ship_spawner::gather_script_data(const game_block &in_block) {
	m_spawn_pos = vector2f(WINDOW_WIDTH * in_block.m_spawn_pos, -OUT_OFFSET);
	std::string tex_name;
	if (in_block.m_type == 0) tex_name = TEX_NAME_ENEMY_00;
	if (in_block.m_type == 1) tex_name = TEX_NAME_ENEMY_01;
	m_cur_tex = resource_module::get_instance()->get_texture(tex_name);
}

void ship_spawner::notify() {
	vector2f forward_vec(0.f, 1.f);
	spaceship *enemy = spawn_spaceship(m_spawn_pos, forward_vec);
	enemy->set_texture(m_cur_tex);
	enemy->on_spawn();
